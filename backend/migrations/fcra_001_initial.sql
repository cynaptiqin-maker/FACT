...
-- ─── 0. Schema upgrade block: add columns missing from pre-existing tables ────
-- Handles the case where FCRA tables were created by an earlier inline DDL version.
-- Each section is guarded by a table-existence check so this is a safe no-op on
-- a fresh database (the tables will be created with all columns below).
DO $$
BEGIN
  -- fcra_bank_accounts
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fcra_bank_accounts' AND table_schema='public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_bank_accounts' AND column_name='is_active') THEN
      ALTER TABLE fcra_bank_accounts ADD COLUMN is_active BOOLEAN NOT NULL DEFAULT true;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_bank_accounts' AND column_name='account_holder') THEN
      ALTER TABLE fcra_bank_accounts ADD COLUMN account_holder VARCHAR(200);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_bank_accounts' AND column_name='opened_date') THEN
      ALTER TABLE fcra_bank_accounts ADD COLUMN opened_date DATE;
    END IF;
  END IF;

  -- fcra_donors
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fcra_donors' AND table_schema='public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_donors' AND column_name='nationality') THEN
      ALTER TABLE fcra_donors ADD COLUMN nationality VARCHAR(100);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_donors' AND column_name='organization_name') THEN
      ALTER TABLE fcra_donors ADD COLUMN organization_name VARCHAR(200);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_donors' AND column_name='is_blocked') THEN
      ALTER TABLE fcra_donors ADD COLUMN is_blocked BOOLEAN NOT NULL DEFAULT false;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_donors' AND column_name='block_reason') THEN
      ALTER TABLE fcra_donors ADD COLUMN block_reason TEXT;
    END IF;
  END IF;

  -- fcra_projects
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fcra_projects' AND table_schema='public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_projects' AND column_name='implementing_agency') THEN
      ALTER TABLE fcra_projects ADD COLUMN implementing_agency VARCHAR(200);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_projects' AND column_name='location') THEN
      ALTER TABLE fcra_projects ADD COLUMN location JSONB NOT NULL DEFAULT '{}';
    END IF;
  END IF;

  -- fcra_receipts
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fcra_receipts' AND table_schema='public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_receipts' AND column_name='financial_year') THEN
      ALTER TABLE fcra_receipts ADD COLUMN financial_year VARCHAR(10);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_receipts' AND column_name='reference_number') THEN
      ALTER TABLE fcra_receipts ADD COLUMN reference_number VARCHAR(100);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_receipts' AND column_name='transaction_type') THEN
      ALTER TABLE fcra_receipts ADD COLUMN transaction_type VARCHAR(20) DEFAULT 'bank_transfer';
    END IF;
  END IF;

  -- fcra_assets
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fcra_assets' AND table_schema='public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_assets' AND column_name='asset_category') THEN
      ALTER TABLE fcra_assets ADD COLUMN asset_category VARCHAR(50);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_assets' AND column_name='fcra_funded_percent') THEN
      ALTER TABLE fcra_assets ADD COLUMN fcra_funded_percent DECIMAL(5,2) NOT NULL DEFAULT 100;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_assets' AND column_name='depreciation_rate') THEN
      ALTER TABLE fcra_assets ADD COLUMN depreciation_rate DECIMAL(5,2) NOT NULL DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_assets' AND column_name='vendor_name') THEN
      ALTER TABLE fcra_assets ADD COLUMN vendor_name VARCHAR(200);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_assets' AND column_name='invoice_number') THEN
      ALTER TABLE fcra_assets ADD COLUMN invoice_number VARCHAR(100);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_assets' AND column_name='disposed_at') THEN
      ALTER TABLE fcra_assets ADD COLUMN disposed_at DATE;
    END IF;
  END IF;

  -- fcra_asset_disposals
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fcra_asset_disposals' AND table_schema='public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_asset_disposals' AND column_name='buyer_address') THEN
      ALTER TABLE fcra_asset_disposals ADD COLUMN buyer_address TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_asset_disposals' AND column_name='mha_approval') THEN
      ALTER TABLE fcra_asset_disposals ADD COLUMN mha_approval BOOLEAN NOT NULL DEFAULT false;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_asset_disposals' AND column_name='mha_reference') THEN
      ALTER TABLE fcra_asset_disposals ADD COLUMN mha_reference VARCHAR(50);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_asset_disposals' AND column_name='proceeds_used_for') THEN
      ALTER TABLE fcra_asset_disposals ADD COLUMN proceeds_used_for TEXT;
    END IF;
  END IF;

  -- fcra_asset_incomes
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fcra_asset_incomes' AND table_schema='public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_asset_incomes' AND column_name='payer_name') THEN
      ALTER TABLE fcra_asset_incomes ADD COLUMN payer_name VARCHAR(200);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_asset_incomes' AND column_name='reference') THEN
      ALTER TABLE fcra_asset_incomes ADD COLUMN reference VARCHAR(100);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_asset_incomes' AND column_name='financial_year') THEN
      ALTER TABLE fcra_asset_incomes ADD COLUMN financial_year VARCHAR(10);
    END IF;
  END IF;

  -- fcra_fc4_filings
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fcra_fc4_filings' AND table_schema='public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_fc4_filings' AND column_name='filing_status') THEN
      ALTER TABLE fcra_fc4_filings ADD COLUMN filing_status VARCHAR(20) NOT NULL DEFAULT 'draft';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_fc4_filings' AND column_name='total_utilized') THEN
      ALTER TABLE fcra_fc4_filings ADD COLUMN total_utilized DECIMAL(20,2) NOT NULL DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_fc4_filings' AND column_name='submitted_date') THEN
      ALTER TABLE fcra_fc4_filings ADD COLUMN submitted_date DATE;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_fc4_filings' AND column_name='acknowledgement_number') THEN
      ALTER TABLE fcra_fc4_filings ADD COLUMN acknowledgement_number VARCHAR(50);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fcra_fc4_filings' AND column_name='mha_remarks') THEN
      ALTER TABLE fcra_fc4_filings ADD COLUMN mha_remarks TEXT;
    END IF;
  END IF;
END $$;
